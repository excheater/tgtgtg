...
async def download_file(file_id: str, dest: str):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        # При --local file_path = абсолютный путь /var/lib/telegram-bot-api/TOKEN/videos/file_0
        # Скачиваем через файл-сервер на порту 8082 который отдаёт /var/lib/telegram-bot-api/
        file_server = LOCAL_API.rstrip('/').replace(':8081', ':8082')

        # Убираем /var/lib/telegram-bot-api/ из пути — файл-сервер уже в этой папке
        relative_path = file_path
        for prefix in ['/var/lib/telegram-bot-api/', 'var/lib/telegram-bot-api/']:
            if relative_path.startswith(prefix):
                relative_path = relative_path[len(prefix):]
                break

        candidates = [
            f"{file_server}/{relative_path}",
            f"{file_server}/{BOT_TOKEN}/{relative_path}",
        ]
        last_status = None
        last_body = None
